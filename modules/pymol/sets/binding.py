...
def _load_cif_text(text, name, _self=cmd):
    _self.load_raw(text, 'cif', name, state=0, finish=1, discrete=-1, quiet=1,
                   multiplex=None, zoom=0)


def _split_cif_blocks(text):
    """The `data_` blocks of a CIF file as (label, text) pairs, in order. Anything
    before the first `data_` line (PyMOL's `# generated by` comment) is dropped: it is
    not a block, and loading it would create nothing."""
    blocks, current, label = [], [], None
    for line in text.splitlines(keepends=True):
        if line.startswith('data_'):
            if label is not None:
                blocks.append((label, ''.join(current)))
            current, label = [line], line[5:].strip()
...
_RUN_MAP = {}   # (store generation, set id, metrics run id) -> set run id


def _run_for(c, set_id, run):
    key = (store.generation(), set_id, run.id)
    ...
    try:
        ...
        if path.lower().endswith(('.cif', '.mmcif')):
            with open(path, errors='replace') as handle:
                blocks = _split_cif_blocks(handle.read())
        if len(blocks) > 1:
            # A set export: one data block per chain. PyMOL would make one object per
            # block, so merge them the way staging does.
            _load_chains([(label, text) for label, text in blocks], PEEK, _self=_self)
        else:
            _self.load(path, PEEK, zoom=0, quiet=1)
    except SetInputError:
        raise
    except Exception as exc:
        raise SetInputError('could not read %s: %s' % (path, exc))
    if not _exists(PEEK, _self=_self):
        # `load` multiplexed into objects named after the file's own blocks.
        raise SetInputError('%s did not load as one object; is it a multi-block CIF'
                            ' with a single chain per block?' % path)
    try:
...
def on_object_renamed(old, new, _self=cmd):
    """After `set_name`: a staged object keeps its link under the new name."""
    if not store.is_open() or not old or old == new:
        return
    c = container()
    for s in c.sets():
...
def save_raymol(filename, quiet=1, _self=cmd):
    ...
    Ordered so that no failure loses data: the copy is made to a temporary beside the
    target, the target and any stale `-wal`/`-shm` of a crashed session are removed
    only when the copy exists, the previous file is deleted only after the new one is
    open and holds the session, and on any failure the previous container is reopened
    and reinstalled.
    """
    ...
    if not same:
        old_path = c.path
        tmp = filename + '.saving'
        store.remove_db_files(tmp)
        try:
            moved = c.save_into(tmp)          # closes c
            moved.close()
            store.remove_db_files(filename)   # the target AND a stale WAL beside it
            os.replace(tmp, filename)
            c = store.replace(store.Container(filename))
        except Exception:
            store.remove_db_files(tmp)
            try:
                store.replace(store.Container(old_path))
            except Exception:
                pass
            raise
    try:
...
def load_raymol(filename, partial=0, quiet=1, *, _self=cmd):
    ...
    Validates everything it can before it changes anything: the file opens as ours and
    its session blob unpickles first, then the scene is cleared or replaced, and only
    then is the document installed and the previous working file removed. A partial
    load is refused: it would merge the scene but there is no meaning to merging two
    documents' sets.
    """
    if int(partial):
        raise SetInputError('a .raymol cannot be loaded partially; export the session'
                            ' as .pse and load that partially instead')
    filename = os.path.abspath(_self.exp_path(filename))
