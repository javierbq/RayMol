...
Expected: all `ok:` lines then `PASS`, including `real repo → X.Y.0` (the actual version depends on what `project.yml` holds; the test asserts structure, not a literal value).
