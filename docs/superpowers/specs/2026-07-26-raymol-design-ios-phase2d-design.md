...
**Simulator (headless).** New `PYMOL_AUTODESIGN` environment hook — the established verification pattern in this codebase, which has 35 such `PYMOL_AUTO*`/`SKIP*` hooks and not one for Design. It enters Design mode, focuses an object, optionally runs a redesign, and writes a completion marker. **Correction (measured 2026-07-26):** this does NOT work. MLX aborts in the iOS 26 Simulator with `MTLStorageModePrivate is required for heaps` on the first inference, and the §6.1 fallback does not prevent it — the abort persists with `Device(.cpu)` as the default and with the cache-limit assignment moved out of that path. MLX's allocator is Metal-backed regardless of compute device, so it builds heaps the Simulator forbids, and MPNNKit requests no device or stream of its own. Isolated to Design mode: a structure loaded with Design never entered produces no assertion. So the Simulator can verify Design's UI, entry points and layout, but **not inference** — anything touching MPNN requires real hardware. The hook remains valuable for macOS and on-device runs. Note `PyMOLViewerUITests` cannot `@testable import` internals (`project.yml:520-526` blanks the bridging header), so it verifies UI and entry points, not logic.
