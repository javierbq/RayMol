...
3. **The long design ops block the UI** via the shared `CalculatingOverlay` ("Redesigning region…" / "Repacking sidechains…"), so no conflicting action can be issued mid-inference (belt-and-suspenders over the job tokens). Driven by `DesignController.designBusyLabel`. A region redesign holds the block through its follow-up rescore + repack (no mid-flow flicker). The quick **initial focus scoring** keeps its lightweight inline spinner (`isScoring`) rather than a full-screen block — gating the whole UI on every object focus would be too aggressive, and `isScoring`'s cross-focus clearing is fragile.
